...
export default class PreviewBlock {
	...
	render() {
		...
		const previewStyle = {
			...
			top: (this.props.data.y - (isBottom && this.props.autoAlign ?
				(codeLines.lines * parseFloat(lineHeight)) : 0)) + 'px',
			width: this.props.data.w + 'px',
			borderColor: window.getComputedStyle($('.wrap-guide', editor.element).get(0)).getPropertyValue('background-color')
		};
		...
		return (
			<div class="folded-code-preview" style={previewStyle}>
				<div class="gutter" innerHTML={codeLines.gutter}></div>
				<div class="code-block" innerHTML={codeLines.html} style={blockStyle}></div>
			</div>
		);
	}
	...
	getCodeLines() {
		...
		const lines = [];
		const iterateEnd = (this.autoAlign > 0 ? foldStart + this.autoAlign : foldEnd);
	...
	processCode(code) {
		return code.replace(/(^|\s)\$1\b/g, '\\$').replace(/&emsp;/g, '$1')
				.replace(/&(\w+;)/g, '&amp;$1').replace(/</ig, '&lt;');
	}
	...
	tokenizeBlock(blockLines, editor, grammar) {
		...
		tokenizedLines.forEach((line, tIndex) => {
			...
			if(line.length === 1 && tIndex === tokenizedLines.length - 1) {
				if(line[0].value.match(/\S/ig) === null)
					return;
			}
		...
		return lineBlocks.map(line => `<div class="line">${line}</div>`).join('');
	}
	...
	generateLineNumbers(start, end, editor) {
		...
		for(let i = start; i <= end; i++) {
			const lineNumberClass = 'line-number ' + (editor.isFoldableAtBufferRow(i - 1) ? 'foldable' : '');

			let lineElement = `<div class="${lineNumberClass}">`;
			const isMultilined = editor.lineTextForBufferRow(i - 2).replace(/\t/g, ' '.repeat(this.tabLength)).length >= this.preferredLineLength;
		...
		const lineNumbersStyle = `padding-right: ${(this.props.data.gutterWidth - $('.gutter', editor.element).outerWidth())}px;`;
	...
	wrapLines(lines) {
		...
		lines.forEach(line => {
			const lineString = line.map(l => l.value).join('').replace(/\t/g, this.repeatString(' ', this.tabLength));
			...
			splits.forEach(split => {
				...
				line.forEach(token => {
					const tokenValue = token.value.replace(/\t/g, this.repeatString(' ', this.tabLength));
	...
	wrapLineText(line) {
		...
		lineSplit.forEach(l => console.log(l));
