...
export default class PreviewBlock {
    ...
    render() {
        ...
        const isBottom =
            this.props.data.y + codeLines.lines * parseFloat(lineHeight) >=
            editor.element.offsetHeight;

        const previewStyle = {
            left: this.props.data.x + "px",
            top:
                this.props.data.y -
                (isBottom && this.props.autoAlign
        ...
        codeLines.html = codeLines.html.replace(/\$1/g, "&emsp;");
    ...
    getCodeLines() {
        ...
        for (let i = foldStart + 2; i <= lastLine; i++) {
            ...
            if (editor.indentationForBufferRow(i - 1) !== indentLevel) continue;

            if (
                editor.isFoldedAtBufferRow(i - 1) &&
                (editor.lineTextForBufferRow(i - 1).trim().length > 0 ||
                    i >= lastLine)
            ) {
                foldEnd = i;
                break;
        ...
        const lines = [];
        const iterateEnd =
            this.autoAlign > 0 ? foldStart + this.autoAlign : foldEnd;
    ...
    tokenizeBlock(blockLines, editor, grammar) {
        const lineBlocks = [];
        const tokenizedLines = this.wrapLines(
            grammar.tokenizeLines(blockLines)
        );

        if (tokenizedLines.length <= 0) return '<span class=""></span>';

        const lineClasses = this.expandScopesToClass(
            tokenizedLines[0][0].scopes[0]
        );
        const indentLevel = editor.indentationForBufferRow(
            this.props.data.screenRow
        );
        ...
        tokenizedLines.forEach((line, tIndex) => {
            ...
            if (line.length === 1 && tIndex === tokenizedLines.length - 1) {
                if (line[0].value.match(/\S/gi) === null) return;
            }
            ...
            line.forEach((token, lIndex) => {
                ...
                const scopes = this.generateLineTokenScopes(
                    token.scopes,
                    tokenValue
                );
            ...
            if (lineElements.length <= 0) {
                const tabString = this.repeatString("&emsp;", this.tabLength);

                for (let i = 0; i < indentLevel + 1; i++)
                    lineElements.push(
                        `<span class="indent-guide">${tabString}</span>`
                    );
            }

            if (lineElements.length > 0)
                lineBlocks.push(
                    `<span class="${lineClasses}">${lineElements.join(
                        ""
                    )}</span>`
                );
        });
    ...
    generateLineTokenScopes(scopes, value) {
        ...
        else if (value.length > 0) e[scopes[0]] = value;
    ...
    tokenScopesToElements(scopes) {
        ...
        for (var prop in scopes) {
            if (!scopes.hasOwnProperty(prop)) continue;
            ...
            if (typeof scopes[prop] === "object")
                ...
                elem += `<span class="${scopeClass}">${this.processCode(
                    scopes[prop]
                )}</span>`;
            else {
                const tabString = this.repeatString("&emsp;", this.tabLength);
                const elementCount = scopes[prop].match(/\t/g).length;
                const tabElements = `<span class="hard-tab leading-whitespace indent-guide">${tabString}</span>`.repeat(
                    elementCount
                );

                if (typeof prop !== "undefined" && prop !== "undefined")
                    elem += `<span class="${scopeClass}">${tabElements}</span>`;
                else elem += tabElements;
            }
        }
    ...
    generateLineNumbers(start, end, editor) {
        const lineElements = [];
        const digitCount = (editor.getLastBufferRow() + 1).toString().length;

        for (let i = start; i <= end; i++) {
            const lineNumberClass =
                "line-number " +
                (editor.isFoldableAtBufferRow(i - 1) ? "foldable" : "");

            let lineElement = `<div class="${lineNumberClass}">`;
            const isMultilined =
                editor
                    .lineTextForBufferRow(i - 2)
                    .replace(/\t/g, " ".repeat(this.tabLength)).length >=
            ...
            lineElement += `${this.repeatString(
                "&nbsp;",
                digitCount - i.toString().length
            )}${i}`;
            lineElement += `<div class="icon-right"></div>`;
            lineElement += `</div>`;
        ...
        if (typeof $ === "jquery") $ = require("jquery");

        const lineNumbersStyle = `padding-right: ${this.props.data.gutterWidth -
            $(".gutter", editor.element).outerWidth()}px;`;

        return `<div class="line-numbers" style="${lineNumbersStyle}">${lineElements.join(
            ""
        )}</div>`;
    }
            ...
            splits.forEach(split => {
                ...
                line.forEach(token => {
                    const tokenValue = token.value.replace(
                        /\t/g,
                        this.repeatString(" ", this.tabLength)
                    );

                    if (
                        (linesSplit[linesSplit.length - 1].length <= 0 &&
                            stringIndex + tokenValue.length >= split.start) ||
                        (stringIndex >= split.start &&
                            stringIndex + tokenValue.length <= split.end)
                    )
                ...
                if (split.start !== 0) {
                    const indentLevel =
                        lineString.match(/\s+/g)[0].length / this.tabLength;

                    for (let i = 0; i < indentLevel; i++) {
                        linesSplit[linesSplit.length - 1].unshift({
                            scopes: ["meta.block.es"],
                            value: "\t"
                        });
                    }
    ...
    wrapLinesToLength(line) {
        ...
        while (line.length > 0 || line.length <= this.preferredLineLength) {
            ...
            if (wrapIndex >= line.length || wrapIndex <= 0) break;
    ...
    findLastIndex(string, regex, mod) {
        ...
        while ((tmp = reg.exec(string)) !== null) lastIndex = reg.lastIndex;

        return lastIndex;
    }

    wrapLineText(line) {
        if (line.length <= this.preferredLineLength) return line;

        lineSplit = line.match(
            new RegExp(".{1," + this.preferredLineLength + "}", "g")
        );
    ...
    expandScopesToClass(scopes) {
        return scopes.split(".").map(c => "syntax--" + c).join(" ");
    }
