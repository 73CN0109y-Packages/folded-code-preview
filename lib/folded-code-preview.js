...
export default {
	...
	activate() {
		$('body').on('mouseenter', '.fold-marker', (e) => {
			...
			const data = {
				y: $e.offset().top - $e.outerHeight() - (lineHeight / 2),
				w: Math.round($('.wrap-guide', editor.element).position().left) + gutterWidth,
				gutterWidth: gutterWidth,
				screenRow: editor.bufferRowForScreenRow(parseInt($e.parent().attr('data-screen-row')))
			};

			if(this.showDelay >= 100)
				this.hoverTimeout = setTimeout(() => this.hoverFoldBlock(data), this.showDelay);
			else
				this.hoverFoldBlock(data);
		});
	...
	hoverFoldBlock(data) {
		...
		atom.workspace.getActiveTextEditor().element.appendChild(this.previewBlockElement.element);
	},

	disposeHoverTimeout() {
		if(typeof this.hoverTimeout === 'undefined')
			return;
	...
	disposeLeaveTimeout() {
		if(typeof this.leaveTimeout === 'undefined')
			return;
